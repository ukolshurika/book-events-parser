...
Текст может быть разбит на страницы, каждая помечена маркером "--- Страница N ---".

Для каждого события определи:
- name: название события
- date: дата события. Это поле обязательно. Если точная дата не указана, выведи ближайший год из контекста с префиксом "около" (например: "около 1774"). Если контекст не даёт никакого ориентира по времени — укажи null. Названия месяцев указывай только в именительном падеже единственного числа: январь, февраль, март, апрель, май, июнь, июль, август, сентябрь, октябрь, ноябрь, декабрь.
- geo: место события или географию (если указано, иначе null)
- page_number: номер страницы из ближайшего маркера "--- Страница N ---" (целое число; null если маркеров нет)

Верни ТОЛЬКО JSON-массив следующей структуры:
[{{"name": "Название события", "date": "Дата", "geo": "Место", "page_number": 5}}, {{"name": "Второе событие", "date": "около 1800", "geo": null, "page_number": 6}}]
